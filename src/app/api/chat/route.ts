...
function buildSystemInstruction(context?: ChatRequest['context']) {
  ...
  const blocks: string[] = [
    `You are a helpful AI tutor for algorithm explanations. Answer in ${language}.`,
    'Use only the provided article content when possible. If the answer is not in the article, say you are not sure.',
    'Return JSON only, no markdown fences.',
    'JSON schema: {"answer":"string","bullets":["string"]}. If there are no bullet points, return an empty bullets array.',
  ];
...
export async function POST(request: Request) {
  ...
  for (const model of FALLBACK_MODELS) {
    ...
    if (result.ok) {
      ...
      let bullets: string[] = [];
      try {
        const parsed = JSON.parse(rawText) as { answer?: string; bullets?: string[] };
