...
const posts: Post[] = [
  {
    ...
    excerpt: {
      ...
      fa: 'با بلوک‌های سازنده بنیادین علوم کامپیوتر آشنا شوید: ساختمان داده‌ها. این پست به معرفی چیستی و چرایی اهمیت آن‌ها می‌پردازد.',
    },
    tags: {
...
This blog will explore these and many other data structures in future posts. Stay tuned!`,
      fa: `ساختمان داده‌ها روشی برای سازماندهی و ذخیره داده‌ها در کامپیوتر است تا بتوان به طور کارآمد به آن‌ها دسترسی داشت و آن‌ها را تغییر داد. این مفهوم تنها به ذخیره داده‌ها محدود نمی‌شود، بلکه به روابط بین نقاط داده و عملیاتی که می‌توان روی داده‌ها انجام داد نیز می‌پردازد.
...
*   **لیست‌های پیوندی:** مجموعه‌ای خطی از عناصر داده که ترتیب آن‌ها بر اساس مکان فیزیکی آن‌ها در حافظه تعیین نمی‌شود.
...
*   **گراف‌ها:** مجموعه‌ای از گره‌ها (یا رأس‌ها) و مجموعه‌ای از یال‌ها که جفت‌هایی از گره‌ها را به هم متصل می‌کنند.
...
این وبلاگ در پست‌های آینده به بررسی این موارد و بسیاری از ساختمان داده‌های دیگر خواهد پرداخت. منتظر باشید!`,
  ...
  },
  {
    ...
    excerpt: {
      ...
      fa: 'یک بررسی عمیق در الگوریتم جستجوی دودویی، یک مفهوم بنیادین در علوم کامپیوتر برای جستجوی کارآمد در آرایه‌های مرتب.',
    },
    tags: {
...
4.  **تکرار.** مراحل 2 و 3 را تا زمانی که عنصر پیدا شود یا بازه خالی شود ادامه دهید.

### مثال کد (TypeScript)

در اینجا یک پیاده‌سازی ساده از جستجوی دودویی در TypeScript آمده است:
...
جستجوی دودویی گواهی بر قدرت استراتژی‌های "تقسیم و غلبه" در الگوریتم‌ها است.`,
  ...
  },
  {
    ...
    title: {
      ...
      fa: 'درک مرتب‌سازی سریع (Quicksort)',
    },
    category: {
      en: 'Sorting',
      fa: 'مرتب‌سازی',
    ...
    excerpt: {
      ...
      fa: 'بررسی Quicksort، یکی از کارآمدترین و پرکاربردترین الگوریتم‌های مرتب‌سازی مبتنی بر پارادایم "تقسیم و غلبه".',
    },
    tags: {
...
Quicksort's performance is sensitive to the choice of pivot. A good pivot selection can help avoid the worst-case scenario.`,
      fa: `Quicksort یک الگوریتم مرتب‌سازی زیبا است که به طور متوسط O(n log n) مقایسه برای مرتب‌سازی n آیتم انجام می‌دهد. در بدترین حالت، O(n²) مقایسه انجام می‌دهد، اگرچه این رفتار نادر است. این یک الگوریتم "تقسیم و غلبه" است.

### طرح پارتیشن‌بندی

کلید Quicksort تابع \`partition()\` است. با داشتن یک آرایه و یک عنصر \`x\` از آرایه به عنوان "محور"، \`x\` را در موقعیت صحیح خود در یک آرایه مرتب شده قرار دهید و تمام عناصر کوچکتر (کوچکتر از \`x\`) را قبل از \`x\`، و تمام عناصر بزرگتر (بزرگتر از \`x\`) را بعد از \`x\` قرار دهید. همه اینها باید در زمان خطی انجام شود.

### مراحل الگوریتم

1.  **یک محور انتخاب کنید.** این می‌تواند اولین عنصر، آخرین، یک عنصر تصادفی، یا میانه باشد.
2.  **پارتیشن‌بندی.** آرایه را به گونه‌ای مرتب کنید که تمام عناصر با مقادیر کمتر از محور قبل از محور قرار گیرند، در حالی که تمام عناصر با مقادیر بیشتر از محور بعد از آن قرار گیرند. پس از این پارتیشن‌بندی، محور در موقعیت مرتب شده نهایی خود قرار دارد.
3.  **بازگشت.** مراحل بالا را به طور بازگشتی روی زیرآرایه عناصر با مقادیر کوچکتر و به طور جداگانه روی زیرآرایه عناصر با مقادیر بزرگتر اعمال کنید.
...
console.log("Sorted array: ", arr); // [1, 5, 7, 8, 9, 10]
\`\`\`
عملکرد Quicksort به انتخاب محور حساس است. انتخاب یک محور خوب می‌تواند به جلوگیری از بدترین سناریو کمک کند.`,
    },
  },
  {
    ...
    excerpt: {
      ...
      fa: 'راهنمایی برای الگوریتم دایکسترا، که کوتاه‌ترین مسیرها را بین گره‌ها در یک گراف وزن‌دار پیدا می‌کند.',
    },
    tags: {
...
Dijkstra's is a cornerstone of graph theory and has applications in network routing protocols, mapping, and logistics.`,
      fa: `الگوریتم دایکسترا یک الگوریتم کلاسیک برای پیدا کردن کوتاه‌ترین مسیرها بین گره‌ها در یک گراف است، که ممکن است به عنوان مثال شبکه‌های جاده‌ای را نمایش دهد. این الگوریتم توسط دانشمند کامپیوتر ادسخر دایکسترا در سال 1956 ابداع شد.
...
1.  **مقداردهی اولیه:**
    *   یک مجموعه از گره‌های بازدید نشده ایجاد کنید، که در ابتدا شامل تمام گره‌ها است.
    *   به هر گره یک مقدار فاصله آزمایشی اختصاص دهید: برای گره اولیه ما آن را صفر و برای سایر گره‌ها بی‌نهایت قرار دهید.
...
2.  **حلقه اصلی:**
    ...
    *   فاصله آزمایشی تازه محاسبه شده را با مقدار اختصاص داده شده فعلی مقایسه کرده و مقدار کوچکتر را اختصاص دهید.
    ...
    *   گره بازدید نشده‌ای را که با کوچکترین فاصله آزمایشی علامت‌گذاری شده است انتخاب کنید، آن را به عنوان "گره فعلی" جدید تنظیم کنید و به مرحله 2 بازگردید.
...
# Output: {'A': 0, 'B': 1, 'C': 3, 'D': 4}
\`\`\`
دایکسترا یک سنگ بنای نظریه گراف است و در پروتکل‌های مسیریابی شبکه، نقشه‌برداری و لجستیک کاربرد دارد.`,
    },
  },
];
